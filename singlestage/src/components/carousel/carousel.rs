...
pub fn Carousel(
    ...
    // TODO: handle decreasing number of items?
    Effect::new(move || {
        let cur = current_item.get();
        let num_items = num_items.get_untracked();
        ...
        if let Some(ul_ref) = ul_ref.get() {
            ...
            ul_ref.set_scroll_left((scroll_step * (cur - 1)).into());
        }
    });
    ...
    view! {
        <div
            class=format!("singlestage-carousel {}", class.get().unwrap_or_default())
