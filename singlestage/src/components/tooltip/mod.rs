...
pub fn Tooltip(
    ...
    view! {
        <div
            data-align=move || {
                let align = align.get().unwrap_or_default();
                if align.is_empty() { "center".to_owned() } else { align }
            }
            data-side=move || {
                let side = side.get().unwrap_or_default();
                if side.is_empty() { "top".to_owned() } else { side }
            }
            data-tooltip=move || value.get()
