        ...
        <form>
            <SidebarGroup class="py-0">
                <SidebarGroupContent class="relative">
                    <Label label_for="search" class="sr-only">
                        "Search"
                    </Label>
                    <InputGroup>
                        <Input id="search" placeholder="Search the docs..." class="pl-8" />
                        <InputGroupAddon>
                            {icon!(
                                icondata::LuSearch, class="pointer-events-none absolute top-1/2 left-2 size-4 -translate-y-1/2 opacity-50 select-none"
                            )}
                        </InputGroupAddon>
                    </InputGroup>
                </SidebarGroupContent>
...
fn AppSidebar() -> impl IntoView {
    ...
    view! {
        <Sidebar>
            ...
            <SidebarContent>
                <For each=move || nav_main.get_value() key=|item| item.title.clone() let(item)>
                    <Collapsible open=true title=item.title.clone()>
                        <SidebarGroup>
                            <CollapsibleTrigger>
                                <SidebarMenuButton>
                                    <Button class="aria-expanded:[&_svg:last-child]:rotate-90">
                                        {item.title} " "
                                        {icon!(
                                            icondata::LuChevronRight, class="ml-auto transition-transform"
                                        )}
                                    </Button>
                                </SidebarMenuButton>
                            </CollapsibleTrigger>
...
#[component]
pub fn SidebarCollapsibleSectionsExample() -> impl IntoView {
